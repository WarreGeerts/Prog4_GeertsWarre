...
namespace ge {
    ...
    TextComponent::TextComponent(GameObject *go, std::string text, std::string fileName, const int fontSize,
        ...
        : Component(go, "TextComponent"), m_needsUpdate(true), m_FontFileName(std::move(fileName)),
    ...
    void TextComponent::SyncReferences() {
        ...
        // Check all components generically
        const auto &components = m_gameObject->GetComponents();
        m_OverWriten = std::any_of(components.begin(), components.end(), [](const auto &comp) {
            return comp->IsOverwritingText();
        });
    }
