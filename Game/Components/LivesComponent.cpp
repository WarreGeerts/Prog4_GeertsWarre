...
namespace game {
	...
	void LivesComponent::Deserialize(const nlohmann::ordered_json& data) {
		for (const auto& handle : m_Handles) {
			if (ge::EventManager::GetInstance().isAlive()) {
				ge::EventManager::GetInstance().DetachEvent(handle);
			}
		}
		m_Handles.clear();
		...
		m_Lives = game::ProgressionManager::GetInstance().GetLives();
	}

	void LivesComponent::PlayerDie() {
		if (m_FramesActive < 3) return;
		...
		--m_Lives;
		game::ProgressionManager::GetInstance().SetLives(m_Lives);
